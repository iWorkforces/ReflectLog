...
class LangchainQwenEmbeddings(BaseModel):
    ...
    def _sync_embed(self, **kwargs: Any) -> list[list[float]]:
        """Call the sync embeddings API once. Retries belong to the SDK."""
    ...
    def embed_documents(self, texts: list[str]) -> list[list[float]]:
        ...
        for i in range(0, len(cleaned_texts), batch_size):
            ...
            batch_embeddings = self._sync_embed(
                input=batch,
                model=self.config.model,
                dimensions=self.config.embedding_dims,
                encoding_format="float",
    ...
    async def _async_embed(self, **kwargs: Any) -> list[list[float]]:
        """Call the async embeddings API once. Retries belong to the SDK."""
        try:
            client = self._get_async_client()
            response = await client.embeddings.create(**kwargs)
        except Exception as exc:
            raise RuntimeError("Async embedding request failed") from exc
        return _ordered_embeddings(response.data)

    async def aembed_query(self, text: str) -> list[float]:
        """Async version: Embed query text with retries."""
        text = text.replace("\n", " ")
        embeddings = await self._async_embed(
            input=[text],
            model=self.config.model,
            dimensions=self.config.embedding_dims,
            encoding_format="float",
        ...
        async def embed_batch(start_idx: int, batch_texts: list[str]) -> None:
            ...
            try:
                async with semaphore:
                    batch_embeddings = await self._async_embed(
                        input=batch_texts,
                        model=self.config.model,
                        dimensions=self.config.embedding_dims,
                        encoding_format="float",
