...
class TestEngineFailure:
    ...
    @pytest.mark.asyncio
    async def test_tantivy_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            self.manager._tantivy_engine,
            "search",
            mock_tantivy_error,
        )

        results = await self.manager.search("test query")

        assert len(results) >= 0, "Should still return results"
        assert len(results) <= self.manager.config.search_limit

    @pytest.mark.asyncio
    async def test_usearch_unavailable(self, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_both_engines_fail(self, monkeypatch):
        """Test search when both engines are unavailable.

        Should raise appropriate error with clear messaging.
        """
        ...
        with pytest.raises(ConnectionError, match="Both search engines unavailable"):
            await self.manager.search("test query")

    @pytest.mark.asyncio
    async def test_llm_reranker_failure(self, monkeypatch):
        ...
        if (
            hasattr(self.manager, "_llm_reranker")
            and self.manager._llm_reranker is not None
        ):
            monkeypatch.setattr(
                self.manager._llm_reranker,
                "rerank",
                mock_reranker_error,
            )
...
class TestResourceExhaustion:
    ...
    @pytest.mark.asyncio
    async def test_disk_space_exhaustion(self, tmp_path):
        ...
        with patch("os.statvfs", mock_disk_full):
            with pytest.raises(OSError):
                await self.manager.add_messages_async(
                    [f"Large message {i}" for i in range(10)]
                )

    @pytest.mark.asyncio
    async def test_memory_exhaustion(self, monkeypatch):
        ...
        await self.manager.add_messages_async([large_text])

        assert True  # Should complete successfully

    @pytest.mark.asyncio
    async def test_connection_pool_exhaustion(self, monkeypatch):
        """Test with limited connection pool.

        Should queue requests when pool is exhausted.
        """
        ...
        for i in range(10):  # Reduced from 100 for faster testing
            try:
                await self.manager.search(f"Query {i}")
...
class TestDataCorruption:
    ...
    @pytest.mark.asyncio
    async def test_invalid_embedding(self, monkeypatch):
        """Test handling of corrupted embedding data.

        Should skip or handle gracefully.
        """
        corrupted_vector = [float("nan")] * self.manager.config.embedding_dims
        ...
        with patch.object(
            ...
            await self.manager.add_messages_async(["test message"])

    @pytest.mark.asyncio
    async def test_config_reload_during_operation(self, monkeypatch):
