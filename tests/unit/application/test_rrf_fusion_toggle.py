...
class TestConcatenateResults:
    """Tests for MemoryManager._concatenate_results() method."""
    ...
    def mock_config(self):
        ...
        config.enable_rrf_fusion = False  # Disabled for concatenation tests
        ...
        config.reranker_engine = "none"  # No reranking for simpler tests
    ...
    def test_concatenate_results_semantic_priority(self, mock_config, mock_logger):
        """Semantic results should come first in concatenation."""
        from ccmemories.application.memory.manager import MemoryManager

        semantic = [("msg1", 0.9), ("msg2", 0.8)]
        tantivy = [("msg3", 0.95), ("msg1", 0.7)]  # msg1 is duplicate
        ...
        with patch("ccmemories.application.memory.manager.USearchEngine"):
            with patch("ccmemories.application.memory.manager.LangchainQwenEmbeddings"):
                with patch("ccmemories.application.memory.manager.TantivyEngine"):
                    ...
                    result = manager._concatenate_results(semantic, tantivy, limit=10)

        # msg1 from semantic (0.9), msg2 from semantic (0.8), msg3 from tantivy
        assert len(result) == 3
        assert result[0] == ("msg1", 0.9)  # Semantic version kept
        assert result[1] == ("msg2", 0.8)
        assert result[2] == ("msg3", 0.95)

    def test_concatenate_results_respects_limit(self, mock_config, mock_logger):
        """Concatenation should respect the limit parameter."""
        from ccmemories.application.memory.manager import MemoryManager

        semantic = [("msg1", 0.9), ("msg2", 0.8), ("msg3", 0.7)]
        tantivy = [("msg4", 0.95), ("msg5", 0.85)]

        with patch("ccmemories.application.memory.manager.USearchEngine"):
            with patch("ccmemories.application.memory.manager.LangchainQwenEmbeddings"):
                with patch("ccmemories.application.memory.manager.TantivyEngine"):
                    manager = MemoryManager(mock_config, mock_logger)
                    result = manager._concatenate_results(semantic, tantivy, limit=3)

        assert len(result) == 3
        assert result[0][0] == "msg1"
        assert result[1][0] == "msg2"
        assert result[2][0] == "msg3"  # Only semantic results fit

    def test_concatenate_results_empty_semantic(self, mock_config, mock_logger):
        """When semantic is empty, only tantivy results returned."""
        from ccmemories.application.memory.manager import MemoryManager

        semantic = []
        tantivy = [("msg1", 0.9), ("msg2", 0.8)]
        ...
        assert len(result) == 2
        assert result[0] == ("msg1", 0.9)
        assert result[1] == ("msg2", 0.8)

    def test_concatenate_results_empty_tantivy(self, mock_config, mock_logger):
        """When tantivy is empty, only semantic results returned."""
        from ccmemories.application.memory.manager import MemoryManager
    ...
    def test_concatenate_results_both_empty(self, mock_config, mock_logger):
        """When both are empty, return empty list."""
        from ccmemories.application.memory.manager import MemoryManager
    ...
    def test_concatenate_results_all_duplicates(self, mock_config, mock_logger):
        """When all tantivy results are duplicates, only semantic returned."""
        from ccmemories.application.memory.manager import MemoryManager

        semantic = [("msg1", 0.9), ("msg2", 0.8)]
        tantivy = [("msg1", 0.7), ("msg2", 0.6)]  # All duplicates
        ...
        # Only semantic results, tantivy duplicates skipped
        assert len(result) == 2
        assert result[0] == ("msg1", 0.9)  # Semantic score preserved
        assert result[1] == ("msg2", 0.8)

    def test_concatenate_results_limit_cuts_off_tantivy(self, mock_config, mock_logger):
        """Limit should cut off tantivy if semantic fills the quota."""
        from ccmemories.application.memory.manager import MemoryManager
        ...
        with patch("ccmemories.application.memory.manager.USearchEngine"):
            with patch("ccmemories.application.memory.manager.LangchainQwenEmbeddings"):
                with patch("ccmemories.application.memory.manager.TantivyEngine"):
                    manager = MemoryManager(mock_config, mock_logger)
                    result = manager._concatenate_results(semantic, tantivy, limit=2)

        # Only semantic results fit
        assert len(result) == 2
        assert result[0] == ("msg1", 0.9)
        assert result[1] == ("msg2", 0.8)
...
class TestSearchPipelineWithRRFToggle:
    ...
    def test_rrf_disabled_uses_concatenation(
