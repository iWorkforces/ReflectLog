...
Test the `openmemories/application/memory/` module in isolation:
...
| File | Tests For |
|------|-----------|
| `test_mcp_server.py` | `openmemories/application/mcp_server.py` |
| `test_memory_manager.py` | `openmemories/application/memory/manager.py` |
| `test_ranx_fusion.py` | `openmemories/application/memory/fusion/ranx_fusion.py` |
| `test_validation.py` | `openmemories/application/utils/validation.py` |
| `test_logging_utils.py` | `openmemories/application/utils/logging.py` |
...
| `memory/reranking/test_normalization.py` | `openmemories/application/memory/reranking/normalization.py` |
| `utils/test_logging.py` | `openmemories/application/utils/logging.py` |
| `utils/test_numba_utils.py` | `openmemories/application/utils/numba_utils.py` |
| `utils/test_security.py` | `openmemories/application/utils/security.py` |
...
def test_usearch_engine_is_initialized():
    ...
    with patch.dict(os.environ, {'PROJECT_ID': 'test'}):
        with patch('openmemories.application.memory.manager.USearchEngine') as mock:
            MemoryManager(config, logger)
...
def test_tantivy_index_creation():
    ...
    with patch.dict(os.environ, {'PROJECT_ID': 'test'}):
        with patch('openmemories.application.memory.manager.tantivy') as mock_tantivy:
            mock_index = Mock()
...
def test_add_logs_with_structured_context():
    ...
    with patch('openmemories.application.mcp_server.get_logger') as mock_logger_func:
        mock_logger = Mock()
...
def mock_memory_manager():
    ...
    with patch('openmemories.application.memory.MemoryManager') as mock_class:
        mock_instance = Mock()
...
# With coverage
uv run pytest tests/unit/application/ --cov=openmemories.application --cov-report=term-missing
```

## Coverage Goals

Aim for **90%+ coverage** of `openmemories/application/mcp_server.py`:
- All `__init__` logic
