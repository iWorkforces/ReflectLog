...
LLM-based reranking for improving search result relevance after RRF fusion.

## ADDED Requirements

### Requirement: LLM Reranker Configuration

The system SHALL support configuration of LLM reranking via environment variables:
- `LLM_MODEL`: LLM model identifier for reranking (default: `x-ai/grok-4.1-fast`)
- `SEARCH_RERANK`: Enable/disable LLM reranking (default: `true`)
...
- **GIVEN** `SEARCH_RERANK=false` is set
...
4. **LLM Reranking** (when `SEARCH_RERANK=true`)
5. Return top `limit` results

#### Scenario: Full hybrid search with reranking
- **GIVEN** hybrid search is enabled and `SEARCH_RERANK=true`
