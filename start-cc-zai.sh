...
# Load environment variables from .env.cc.zai
SCRIPT_DIR="${0:A:h}"
ENV_FILE="${SCRIPT_DIR}/.env.cc.zai"

if [[ ! -f "$ENV_FILE" ]]; then
    echo "Error: .env.cc.zai not found at $ENV_FILE" >&2
    echo "" >&2
    echo "To get started, copy an example file:" >&2
    echo "  cp .env.cc.zai.example .env.cc.zai   # or .env.cc.zai.zai.example" >&2
    echo "Then edit .env.cc.zai with your API keys." >&2
    exit 1
fi
...
Environment:
    Loads configuration from .env.cc.zai in the script directory.
